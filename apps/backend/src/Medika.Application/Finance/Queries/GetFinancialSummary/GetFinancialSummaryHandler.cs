    ...
    ICurrentUserService currentUser) : ICommandHandler<GetFinancialSummaryQuery, FinancialSummary>
{
    public async Task<FinancialSummary> ExecuteAsync(GetFinancialSummaryQuery query, CancellationToken ct)
    {
        ...
        var income = await invoices.SumByPeriodAsync(cabinetId, currentUser.UserId, monthStart, monthEnd, ct);
        var chargesTotal = await charges.SumByPeriodAsync(cabinetId, currentUser.UserId, monthStart, monthEnd, ct);
        var pendingInvoices = await invoices.GetByStatusAsync(cabinetId, InvoiceStatus.Pending, currentUser.UserId, ct);
        var monthInvoices = await invoices.GetByPeriodAsync(cabinetId, currentUser.UserId, monthStart, monthEnd, ct);
        ...
        for (int i = 5; i >= 0; i--)
        {
            ...
            var mIncome = await invoices.SumByPeriodAsync(cabinetId, currentUser.UserId, m, mEnd, ct);
