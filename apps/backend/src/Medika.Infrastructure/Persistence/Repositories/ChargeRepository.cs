...
{
    public async Task<IReadOnlyList<Charge>> GetByPeriodAsync(string doctorId, DateOnly from, DateOnly to, CancellationToken ct = default) =>
    ...
    {
        var charges = await GetByPeriodAsync(doctorId, from, to, ct);
        return charges.Sum(c => c.Amount);
    }

    public async Task<IReadOnlyList<Charge>> GetByDoctorAndMonthAsync(string doctorId, int year, int month, CancellationToken ct = default)
    {
