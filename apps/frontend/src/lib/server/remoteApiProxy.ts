...
export class RemoteApi {
	 ...
	 */
	private static async buildAndFetch(args: {
		url: string;
		options: RequestInit;
		token?: string;
	}): Promise<{ response: Response; fullUrl: string }> {
		const headers: Record<string, string> = {
			'X-API-KEY': env.API_SECRET ?? '',
			'X-Request-Timestamp': new Date().toISOString(),
			...(args.options.body !== undefined && !(args.options.body instanceof FormData)
	 ...
	 */
	static async fetchThenRetrieveJson<T>(args: {
		url: string;
		options?: RequestInit;
		token?: string;
	}): Promise<T> {
